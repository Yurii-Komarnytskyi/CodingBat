...
public class ArrayTwo {
	...
	public int sum13(int[] nums) {
		if (nums.length - 1 == -1)
			return 0;
		List<Integer> nl = Arrays.stream(nums).boxed().collect(Collectors.toCollection(ArrayList::new));
	...
	public boolean either24(int[] nums) {
		List<Integer> numsList = Arrays.stream(nums).boxed().collect(Collectors.toCollection(ArrayList::new));
	...
	public boolean has12(int[] nums) {
		List<Integer> numsList = Arrays.stream(nums).boxed().collect(Collectors.toCollection(ArrayList::new));
		return numsList.lastIndexOf(2) > numsList.indexOf(1) && 
		(numsList.contains(1) && numsList.contains(2));
	}
	...
	public boolean sameEnds(int[] nums, int len) {
		List<Integer> numsCollection = Arrays.stream(nums).boxed().collect(Collectors.toCollection(ArrayList::new));
	...
	public int[] tenRun(int[] nums) {
		boolean hasMultOfTen = Arrays.stream(nums)
			.anyMatch(n -> n % 10 == 0);
		if( !hasMultOfTen)
			return nums;
	...
	public int[] pre4(int[] nums) {
		...
		if(firstFourIndex == -1) 
			return nums;
		return numsCollection.subList(0, firstFourIndex).stream()
	...
	public int[] post4(int[] nums) {
		...
		return numsCollection.subList(lastFourIndex + 1, numsCollection.size()).stream()
			.mapToInt(Integer::intValue)
			.toArray();
	}
